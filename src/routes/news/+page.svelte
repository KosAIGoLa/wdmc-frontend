...
{#if c}
	<BannerCarousel
		banners={news.banners.map((b, i) => ({
			src: b.src,
			alt: c.news.bannersAlt[i] ?? ''
		}))}
	/>
		...
		<div class="container-page">
			...
			<div class="grid gap-6 md:grid-cols-2">
				{#each visibleItems as item (item.id)}
					{@const text = c.news.items[item.id as keyof typeof c.news.items]}
					<article class="card group min-h-[214px] p-5">
						<div class="flex h-full gap-5">
							<a
								href={resolve(`/news/${item.id}`)}
								class="media-frame block h-[120px] w-[120px] shrink-0 rounded-lg sm:h-[126px] sm:w-[126px]"
							>
								<img src={asset(item.image)} alt={text?.title ?? ''} loading="lazy" />
							</a>
